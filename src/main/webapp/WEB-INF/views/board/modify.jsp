                    ...
                    <div class="card shadow mb-4">
                        ...
                        <div class="card-body">
                            <form method="post" id="modifyform">
							    ...
							    <div class="uploadResult my-3">
							    	...
							    	<c:forEach items="${board.attachs}" var="attach">
							    		<c:if test="${not empty board.attachs[0].uuid}">
								    		...
								    		<i class="far fa-times-circle btn-remove float-right" data-uuid="${attach.uuid}" data-file="${attachs.url}&thumb=off"></i>
								    		</li>
							    		</c:if>
							    	</c:forEach>
							    	</ul>
							  ...
							  <sec:authorize access="isAuthenticated() and principal.username eq #board.writer"><!-- #board EL문법에 있는 board를 따라감 -->
...
$(function() {
   ...
   $("form button[formaction='modify']").click(function () {
      ...
      console.log(str);
      $("#modifyform").attr("action","modify");//모델 어트리뷰트에서 파라미터 2개를 받을때 이슈때문에 추가해야된다
      $("#modifyform").append(str);
      console.log($("#modifyform").serialize());
      console.log($("#modifyform").serializeArray());//이건 배열로 보여줌
      $("#modifyform").submit();
   })
