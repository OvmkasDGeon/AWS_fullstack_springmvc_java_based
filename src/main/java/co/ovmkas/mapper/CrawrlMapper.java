...
public interface CrawrlMapper {
	...
	@Select("select * from t_cgv where pidx = #{pdix}")
	Map<String, Object> select(String pidx);
	
	@Insert("insert into t_person(pidx, name, href) values(#{pidx}, #{name}, #{href}) ON DUPLICATE KEY UPDATE name = name")//insert 시 에러메시지를 안보이기 위함
	...
	void insertAttahc(Map<String, String> map);
