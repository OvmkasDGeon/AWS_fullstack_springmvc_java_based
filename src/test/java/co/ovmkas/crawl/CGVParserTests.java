...
import co.ovmkas.mapper.CrawrlMapper;
...
public class CGVParserTests {
	...
	public void parse() throws MalformedURLException, IOException{
		Document doc = Jsoup.parse(new URL("http://www.cgv.co.kr/movies/"), 2000);//2000은 시간?
...
//		InputStream is = new URL("https://img.cgv.co.kr/Movie/Thumbnail/Poster/000086/86153/86153_320.jpg").openStream();
//		FileUtils.copyInputStreamToFile(is, new File("c:/img/tmp.png"));
		int i=0;
		//Stream<String>
		...
		for(Map<String, Object> m :thumbs){
			...
			File file = new File("c:/testimg",m.get("midx").toString());
			if(!file.exists()){
				file.mkdirs();
			}
...
//			}
	...
	public void testDownloadStillCut() throws MalformedURLException, IOException {
		...
		for (Map<String, Object> m : list) {
			String url = "http://www.cgv.co.kr" + m.get("href");
			Document doc = Jsoup.parse(new URL(url), 2000);// 2000은 응답 대기 시간? 상세페이지
			//타임아웃 문제 시 Jsoup.connect(url).get();
